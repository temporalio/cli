...
func (v *ClientOptions) BuildFlags(f *pflag.FlagSet) {
	...
	f.BoolVar(&v.TlsDisableHostVerification, "tls-disable-host-verification", false, "Disable TLS host-name verification. Enables TLS if it is not already enabled.")
	f.StringVar(&v.TlsServerName, "tls-server-name", "", "Override target TLS server name. Enables TLS if it is not already enabled.")
