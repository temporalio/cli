...
func NewTemporalCommand(cctx *CommandContext) *TemporalCommand {
	...
	s.Command.PersistentFlags().BoolVar(&s.NoJsonShorthandPayloads, "no-json-shorthand-payloads", false, "Show all payloads as raw, even if they are JSON.")
	s.initCommand(cctx)
	return &s
}
