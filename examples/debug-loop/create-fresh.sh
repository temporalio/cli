...
func runRaceScenario(c client.Client, namespace, orderID string, ts int64, wait bool) {
	...
	if failedID != "" {
		...
		log.Println("")
		log.Printf("  temporal agent trace --workflow-id %s --namespace %s -o json", failedID, namespace)
		log.Printf("  temporal agent timeline --workflow-id %s --namespace %s -o json", failedID, namespace)
		log.Println("")
